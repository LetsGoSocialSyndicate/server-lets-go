...
const { getValidToken } = require('../utilities/jwtUtil')
    ...
    userService.getByEmail(email).then((result) => {
      ...
      if (bcrypt.compareSync(password, result.hashed_password)) {
        if (result.verified_at) {
          const token = generateToken({ email, userId: result.id })
      ...
      let payload = constructFailure(
        DATABASE_ERROR,
        'Error while looking up user in database',
        status
      )
      if (err.isBoom) {
        ...
        if (status === 404) {
          payload = constructFailure(
            BAD_EMAIL_OR_PASSWORD, 'Bad email or password.', 401
          )
        } else {
...
router.get('/refresh', (req, res, next) => {
  try {
    ...
    // console.log('refresh OLD:', token)
    const newToken = generateToken({ email: token.email, userId: token.userId })
    // console.log('refresh NEW:', newToken)
    res.status(200).json({ token: newToken })
  } catch (error) {
    next(constructFailure(BAD_TOKEN, error, 403))
  }
})
...
router.get('/:token', (req, res, next) => {
  ...
  tokenService.get(token).then(resultTokenRecord => {
    ...
    if (tokenAge > ONE_DAY) {
      tokenService.delete(token).then(() => {
